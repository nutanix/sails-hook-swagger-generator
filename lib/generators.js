 ...
 * @private
 */
function _generatePaths(generated_routes, tags, swaggerConfig) {
    var paths = {};
    ...
    _.forEach(generated_routes, function (routes, identity) {
        _.forEach(routes, function (route) {
            ...
            // Extend route configs with custom swagger document properties
            if (swaggerConfig.swagger &&
                swaggerConfig.swagger.paths &&
                swaggerConfig.swagger.paths[route.http_method + ' ' + route.path]
            ) {
                _.assign(route, swaggerConfig.swagger.paths[route.http_method + ' ' + route.path])
            }
            

            identity = identity && identity.split('/').pop()
          ...
          if(!route.custom){ // if not from routes.js use the default custom blueprint maps
            parameter = formatter.blueprint_parameter(route.action, swaggerConfig.customBlueprintMaps);
          }
            ...
            var path = {
                summary: route.summary ? route.summary.replace("{identity}", identity) : '',
                description: route.description ? route.description.replace("{identity}", identity) : '',
                tags: tag ? [tag.name] : [identity],
                produces: route.produces || ['application/json'],
                consumes: route.consumes || ['application/json'],
            ...
            if (route.action === "create" || route.action === "update") {
              ...
              if(!_.find(path.parameters, {name: 'body'})){
                path.parameters.push({
                    ...
                    "in": "body",
                    "required": true,
                    "description": "An object defining our schema for " + identity,
